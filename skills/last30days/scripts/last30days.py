...
def run_research(
    ...
    Returns:
        Tuple of (reddit_items, x_items, web_items, web_needed, raw_openai, raw_xai, raw_reddit_enriched, raw_web, reddit_error, x_error, web_error)
    ...
    if sources == "web":
        ...
        if config.get("PERPLEXITY_API_KEY"):
            try:
                raw_web = perplexity_web.search_web(
                    config["PERPLEXITY_API_KEY"],
                    topic,
                    from_date,
                    to_date,
                    max_results=15 if depth == "deep" else (8 if depth == "quick" else 12),
                    model=config.get("PERPLEXITY_MODEL", "sonar"),
                )
                web_items = websearch.parse_websearch_results(
                    perplexity_web.parse_perplexity_response(raw_web),
                    topic,
                    from_date,
                    to_date,
                )
                ...
                raw_web = {"error": str(e)}
                web_error = f"API error: {e}"
            except Exception as e:
                raw_web = {"error": str(e)}
                web_error = f"{type(e).__name__}: {e}"
        if progress:
            progress.end_web_only(source="Perplexity" if config.get("PERPLEXITY_API_KEY") else "Claude", count=len(web_items))
        return reddit_items, x_items, web_items, web_needed, raw_openai, raw_xai, raw_reddit_enriched, raw_web, reddit_error, x_error, web_error
...
def main():
    ...
    parser.add_argument(
        "--sources",
        choices=["auto", "reddit", "x", "both", "web"],
        default="auto",
    ...
    # Run research
    reddit_items, x_items, web_items, web_needed, raw_openai, raw_xai, raw_reddit_enriched, raw_web, reddit_error, x_error, web_error = run_research(
        args.topic,
        sources,
        config,
        selected_models,
    ...
    normalized_web = websearch.normalize_websearch_items(web_items, from_date, to_date)
    ...
    # Write outputs
    render.write_outputs(report, raw_openai, raw_xai, raw_reddit_enriched, raw_web)

    # Show completion
    if sources == "web":
        progress.show_web_only_complete(
            source="Perplexity" if (config.get("PERPLEXITY_API_KEY") and not web_needed) else "Claude",
            count=len(deduped_web),
        )
    else:
