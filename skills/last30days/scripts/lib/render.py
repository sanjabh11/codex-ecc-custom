...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    if report.mode == "web-only":
        if report.web:
            lines.append("**🌐 WEB SEARCH MODE** - Results fetched via Perplexity")
        else:
...
def write_outputs(
    ...
    if raw_web:
        with open(OUTPUT_DIR / "raw_web.json", 'w') as f:
            json.dump(raw_web, f, indent=2)
