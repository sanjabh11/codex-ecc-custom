...
class ProgressDisplay:
    ...
    def end_web_only(self, source: str = "Claude", count: int = 0):
        """End web-only spinner."""
        if self.spinner:
            if source == "Perplexity":
                self.spinner.stop(f"{Colors.GREEN}Web{Colors.RESET} {source} returned {count} pages")
            else:
                self.spinner.stop(f"{Colors.GREEN}Web{Colors.RESET} Claude will search the web")

    def show_web_only_complete(self, source: str = "Claude", count: int = 0):
        """Show completion for web-only mode."""
        elapsed = time.time() - self.start_time
        if IS_TTY:
            if source == "Perplexity":
                sys.stderr.write(f"\n{Colors.GREEN}{Colors.BOLD}✓ Web research complete{Colors.RESET} ")
            else:
                sys.stderr.write(f"\n{Colors.GREEN}{Colors.BOLD}✓ Ready for web search{Colors.RESET} ")
            sys.stderr.write(f"{Colors.DIM}({elapsed:.1f}s){Colors.RESET}\n")
            if source == "Perplexity":
                sys.stderr.write(f"  {Colors.GREEN}Web:{Colors.RESET} {count} pages from Perplexity\n\n")
            else:
                sys.stderr.write(f"  {Colors.GREEN}Web:{Colors.RESET} Claude will search blogs, docs & news\n\n")
        else:
            if source == "Perplexity":
                sys.stderr.write(f"✓ Web research complete ({elapsed:.1f}s) - Web: {count} pages from Perplexity\n")
            else:
